...
if board is None:
   print("Плата не найдена. Проверьте кабель и запишите на плату прошивку задания.")
else:
   print("Плата на порту " + board.device)
   print("Нажмите кнопку %d раз — у вас %d секунд" % (EXPECTED_LINES, DURATION_S))
   lines = listen(board)
   write_log(board, lines)
   print("Принято строк: %d, лог записан в %s" % (len(lines), LOG_NAME))
